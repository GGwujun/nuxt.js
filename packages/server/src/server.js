...
export default class Server {
  ...
  _normalizeMiddleware(middleware) {
    ...
    if (!middleware.handle) {
      middleware.handle = (req, res, next) => {
        next(
          new Error(
            "ServerMiddleware should expose a handle: " + middleware.entry
          )
        );
      };
    }
    ...
    if (
      middleware.handle.prefix !== undefined &&
  ...
  replaceMiddleware(query, middleware) {
    ...
    if (typeof query === "string") {
      // Search by entry
      serverStackItem = this.app.stack.find(
        ({ handle }) => handle._middleware && handle._middleware.entry === query
      );
    } else {
      // Search by reference
  ...
  serverMiddlewarePaths() {
    return this.app.stack
      .map(({ handle }) => handle._middleware && handle._middleware.entry)
      .filter(Boolean);
  }
