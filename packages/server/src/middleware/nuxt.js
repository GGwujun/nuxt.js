...
export default ({ options, nuxt, renderRoute, resources }) =>
...
>>>>>>> feab4516934266ce2760c08ad3d9eb33712f0760
      ...
      if (!error && options.render.etag) {
        const { hash } = options.render.etag;
        const etag = hash
          ? hash(html, options.render.etag)
          : generateETag(html, options.render.etag);
        if (fresh(req.headers, { etag })) {
          res.statusCode = 304;
          await nuxt.callHook("render:beforeResponse", url, result, context);
          res.end();
...
const defaultPushAssets = (preloadFiles, shouldPush, publicPath, options) => {
  if (shouldPush && options.dev) {
    consola.warn(
      "http2.shouldPush is deprecated. Use http2.pushAssets function"
    );
  }
...
const getCspString = ({
  ...
  const policyObjectAvailable =
    typeof policies === "object" &&
    policies !== null &&
    !Array.isArray(policies);

  if (Array.isArray(allowedSources) && allowedSources.length) {
    return isReportOnly && policyObjectAvailable && !!policies["report-uri"]
      ? `${baseCspStr} ${allowedSources.join(" ")}; report-uri ${
          policies["report-uri"]
        };`
      : `${baseCspStr} ${allowedSources.join(" ")}`;
  }

  if (policyObjectAvailable) {
    const transformedPolicyObject = transformPolicyObject(
      policies,
      cspScriptSrcHashes
    );
    return Object.entries(transformedPolicyObject)
...
const transformPolicyObject = (policies, cspScriptSrcHashes) => {
  const userHasDefinedScriptSrc =
    policies["script-src"] && Array.isArray(policies["script-src"]);

  const additionalPolicies = userHasDefinedScriptSrc
    ? policies["script-src"]
    : [];

  // Self is always needed for inline-scripts, so add it, no matter if the user specified script-src himself.
  const hashAndPolicyList = cspScriptSrcHashes.concat(
    "'self'",
    additionalPolicies
  );
