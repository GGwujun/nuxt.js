...
export default class Builder {
  constructor(nuxt, bundleBuilder) {
    ...
    this.supportedExtensions = [
      "vue",
      "js",
      ...(this.options.build.additionalExtensions || [])
    ];
  ...
  async build() {
    ...
    if (this.options.dev) {
      ...
      if (this.options.render.ssr) {
        consola.info(
          `Bundling for ${chalk.bold.yellow("server")} and ${chalk.bold.green(
            "client"
          )} side`
        );
      } else {
    ...
    this.options.build.watch.push(
      this.globPathWithExtensions(this.template.dir)
    );
  ...
  async validatePages() {
    ...
    if (
      ...
      (await fsExtra.exists(
        path.join(this.options.srcDir, this.options.dir.pages)
      ))
    ) {
      return;
    }
    ...
    consola.warn(
      `No \`${this.options.dir.pages}\` directory found in ${dir}. Using the default built-in page.`
    );
  }
  ...
  validateTemplate() {
    ...
    for (const depName in templateDependencies) {
      ...
      const pkg = this.nuxt.resolver.requireModule(
      ...
      if (pkg) {
        ...
        if (!validVersion) {
          consola.warn(
            `${depName}@${depVersion} is recommended but ${depName}@${pkg.version} is installed!`
          );
        }
      } else {
  ...
  async normalizePlugins() {
    ...
    return uniqBy(
      this.options.plugins.map(p => {
        ...
        const pluginBaseName = path
          .basename(p.src, path.extname(p.src))
        ...
        if (p.ssr === false) {
          ...
          consola.warn(
            `Invalid plugin mode (server/client/all): '${p.mode}'. Falling back to 'all'`
          );
          p.mode = "all";
        }
  ...
  async resolveFiles(dir, cwd = this.options.srcDir) {
    return this.ignore.filter(
      await glob(this.globPathWithExtensions(dir), {
        cwd,
  ...
  async resolveRelative(dir) {
    ...
    return (await this.resolveFiles(dir)).map(file => ({
  ...
  async resolveLayouts({ templateVars, templateFiles }) {
    ...
    if (
      await fsExtra.exists(
        path.resolve(this.options.srcDir, this.options.dir.layouts)
      )
    ) {
      for (const file of await this.resolveFiles(this.options.dir.layouts)) {
        const name = file
          ...
          .replace(
            new RegExp(`\\.(${this.supportedExtensions.join("|")})$`),
            ""
          );
        ...
        if (name === "error") {
          ...
          consola.warn(
            `Duplicate layout registration, "${name}" has been registered as "${this.options.layouts[name]}"`
          );
  ...
  async resolveStore({ templateVars, templateFiles }) {
    ...
    templateVars.storeModules = (
      await this.resolveRelative(this.options.dir.store)
    ).sort(({ src: p1 }, { src: p2 }) => {
  ...
  async resolveMiddleware({ templateVars, templateFiles }) {
    ...
    templateVars.middleware = middleware.map(({ src }) => {
      ...
      const dst = this.relativeToBuild(
        this.options.srcDir,
        this.options.dir.middleware,
        src
      );
      return { name, src, dst };
    });
  ...
  async resolveCustomTemplates(templateContext) {
    ...
    templateContext.templateFiles = await Promise.all(
      templatePaths.map(async file => {
        ...
        const customTemplate =
          customTemplateIndex !== -1
            ? this.options.build.templates[customTemplateIndex]
            : null;
        let src = customTemplate
          ? customTemplate.src || customTemplate
          : r(this.template.dir, file);

        // Allow override templates using a file with same name in ${srcDir}/app
        const customAppFile = path.resolve(
          this.options.srcDir,
          this.options.dir.app,
          file
        );
        const customAppFileExists =
          customAppFile.startsWith(appDir) &&
          (await fsExtra.exists(customAppFile));
  ...
  watchClient() {
    ...
    patterns = patterns.map((path, ...args) =>
      upath.normalizeSafe(this.globPathWithExtensions(path), ...args)
    );
    ...
    this.createFileWatcher(
      ...
      ["change"],
      refreshFiles,
      this.assignWatcher("custom")
    );

    // Watch for app/ files
    this.createFileWatcher(
      [r(this.options.srcDir, this.options.dir.app)],
      ["add", "change", "unlink"],
      refreshFiles,
      this.assignWatcher("app")
    );
  }
      ...
      debounce((event, fileName) => {
        ...
        for (const entry of dep2Entry[fileName]) {
          ...
          if (!newItem) {
            ...
            return this.nuxt.callHook("watch:restart", {
              event,
              path: fileName
            });
          }
          ...
          // Log
          consola.info(
            `[HMR] ${chalk.cyan(newItem.route || "/")} (${chalk.grey(
              fileName
            )})`
          );
        }
  ...
  watchRestart() {
    ...
    if (!this.options.store) {
      nuxtRestartWatch.push(
        path.join(this.options.srcDir, this.options.dir.store)
      );
    }
