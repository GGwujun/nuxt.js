...
export default class NuxtCommand extends Hookable {
  ...
  async run() {
    ...
    if (typeof this.cmd.run !== "function") {
      throw new TypeError(
        "Invalid command! Commands should at least implement run() function."
      );
    }
  ...
  isUserSuppliedArg(option) {
    return (
      this._argv.includes(`--${option}`) ||
      this._argv.includes(`--no-${option}`)
    );
  }

  _getDefaultOptionValue(option) {
    return typeof option.default === "function"
      ? option.default(this.cmd)
      : option.default;
  }
  ...
  _getHelp() {
    ...
    for (const name in this.cmd.options) {
      ...
      let optionHelp = "--";
      optionHelp +=
        option.type === "boolean" && this._getDefaultOptionValue(option)
          ? "no-"
          : "";
      optionHelp += name;
      if (option.alias) {
    ...
    const usage = foldLines(
      `Usage: nuxt ${this.cmd.usage} [options]`,
      startSpaces
    );
