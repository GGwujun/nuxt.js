...
>>>>>>> feab4516934266ce2760c08ad3d9eb33712f0760
  ...
  async _ready() {
    ...
    // -- Development mode --
    if (this.options.dev) {
      this.serverContext.nuxt.hook("build:resources", mfs =>
    ...
    if (!this.options._start) {
      this.serverContext.nuxt.hook("build:resources", () =>
  ...
  async loadTemplates() {
    ...
    if (await fs.exists(errorTemplatePath)) {
      ...
      this.serverContext.resources.errorTemplate = this.parseTemplate(
        errorTemplate
      );
    }
    ...
    if (await fs.exists(loadingHTMLPath)) {
      this.serverContext.resources.loadingHTML = await fs.readFile(
        loadingHTMLPath,
        "utf8"
      );
      this.serverContext.resources.loadingHTML = this.serverContext.resources.loadingHTML.replace(
        /\r|\n|[\t\s]{3,}/g,
        ""
      );
    } else {
  ...
  get isReady() {
    ...
    if (
      this.SSR &&
      (!this.serverContext.resources.ssrTemplate || !this.renderer.ssr)
    ) {
      return false;
    }
  ...
  detectModernBuild() {
    ...
    consola.info(
      `Modern bundles are detected. Modern mode (\`${options.modern}\`) is enabled now.`
    );
  }
  ...
  createRenderer() {
    ...
    if (
      this.serverContext.resources.ssrTemplate &&
  ...
  renderSSR(renderContext) {
    ...
    const renderer = renderContext.modern
      ? this.renderer.modern
      : this.renderer.ssr;
  ...
  async renderRoute(url, renderContext = {}, _retried = 0) {
    ...
    if (!this.isReady) {
      ...
      switch (this._state) {
        case "created":
          throw new Error(
            "Renderer ready() is not called! Please ensure `nuxt.ready()` is called and awaited."
          );
        case "loading":
        ...
        case "ready":
          throw new Error(
            `Renderer resources are not loaded! Please check possible console errors and ensure dist (${this.distPath}) exists.`
          );
        default:
...
>>>>>>> feab4516934266ce2760c08ad3d9eb33712f0760
    ...
    if (renderContext.modern === undefined) {
      ...
      renderContext.modern =
        modernMode === "client" || isModernRequest(req, modernMode);
    }

    // Set runtime config on renderContext
    renderContext.runtimeConfig = {
      private: renderContext.spa
        ? {}
