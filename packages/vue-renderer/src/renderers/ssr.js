...
export default class SSRRenderer extends BaseRenderer {
  get rendererOptions() {
    const hasModules = fs.existsSync(
...
>>>>>>> feab4516934266ce2760c08ad3d9eb33712f0760
  ...
  async render(renderContext) {
    ...
    const containsUnsafeInlineScriptSrc =
      csp.policies &&
      csp.policies["script-src"] &&
      csp.policies["script-src"].includes("'unsafe-inline'");
    const shouldHashCspScriptSrc =
      csp && (csp.unsafeInlineCompatibility || !containsUnsafeInlineScriptSrc);
    const inlineScripts = [];
...
>>>>>>> feab4516934266ce2760c08ad3d9eb33712f0760
      ...
      const stateScript = `window.${
        this.serverContext.globals.context
      }=${devalue({
        staticAssetsBase,
        ...state
      })};`;
...
<<<<<<< HEAD
      ...
      const payloadScript = `__NUXT_JSONP__("${routePath}", ${devalue({
        data,
        fetch,
        mutations
      })});`;
...
>>>>>>> feab4516934266ce2760c08ad3d9eb33712f0760
      ...
      if (shouldInjectScripts || shouldHashCspScriptSrc) {
        ...
        serializedSession = `window.${
          this.serverContext.globals.context
        }=${devalue(renderContext.nuxt)};`;
    ...
    const templateParams = {
      HTML_ATTRS: meta
        ? meta.htmlAttrs.text(
            renderContext.nuxt.serverRendered /* addSrrAttribute */
          )
        : "",
